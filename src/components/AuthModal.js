  ...
  return (
    <form onSubmit={handleSignup} className="auth-form">
      ...
      <div className="auth-form-field">
        ...
        />
        {formErrors.email && (
          <p className="form-error-field">{formErrors.email}</p>
        )}
      </div>
      ...
      <div className="auth-form-field">
        ...
        />
        {formErrors.username && (
          <p className="form-error-field">{formErrors.username}</p>
        )}
      </div>
      <div className="auth-form-field">
        <input
        ...
        />
        {formErrors.password && (
          <p className="form-error-field">{formErrors.password}</p>
        )}
      </div>
      <div className="auth-form-field">
        <input
        ...
        {formErrors.passwordConfirm && (
          <p className="form-error-field">{formErrors.passwordConfirm}</p>
        )}
      </div>
      <button className="submitButton" type="submit">
        Sign Up
      </button>
      <span className="switch-modal-info">
        All ready have an account?{" "}
        <a href="#" onClick={showLoginForm} className="switch-modal-link">
          Log In
        </a>
      </span>
    </form>
...
const LoginForm = ({
  ...
  return (
    <form onSubmit={handleLogin} className="auth-form">
      ...
      <span className="switch-modal-info">
        No account?{" "}
        <span onClick={showSignupForm} className="switch-modal-link">
          Sign Up
        </span>
      </span>
    </form>
...
class AuthModal extends Component {
  ...
  handleLogin = e => {
    ...
    if (!this.anyFormErrors()) {
      axios
        .post(`${process.env.REACT_APP_SERVER_URL}/authenticate/login`, {
          username,
          password
        })
        .then(response => {
        ...
        .catch(error => {
          ...
          if (error.response.status === 500) {
            loginError =
              "We are having some issues at the moment. Please try again later or contact teamplayer4321234@gmail.com for assistance.";
          } else {
  ...
  handleSignup = event => {
    ...
    if (!this.anyFormErrors()) {
      axios
        .post(`${process.env.REACT_APP_SERVER_URL}/authenticate/signup`, {
          email,
          username,
          password,
          passwordConfirm
